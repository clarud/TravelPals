...
const GoogleOAuthButton = ({ 
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Step 1: Get Google OAuth URL from your backend
      const { url } = await authService.loginWithOAuth('google')
      
      console.log('Received OAuth URL:', url)
      
      // Step 2: Redirect user to Google OAuth
      // We'll store a callback handler for when they return
  ...
  const buttonText = text || (variant === 'signup' ? 'Sign up with Google' : 'Sign in with Google')
